...
export default function useCommandHandler(handlers: CommandHandlers) {
    ...
    const handlersRef = useRef({ ...builtInHandlers, ...createContextHandlers(appContext), ...handlers });
    handlersRef.current = { ...builtInHandlers, ...createContextHandlers(appContext), ...handlers };
